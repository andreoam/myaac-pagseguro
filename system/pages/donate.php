...
if (empty($action)) {
    if (!$logged) {
        ...
        global $twig_loader;
        $twig_loader->prependPath(BASE . 'plugins/pagseguro');

        if (isset($_REQUEST['step']) && $_REQUEST['step'] == "accept") {
            // Aqui você adiciona a lógica para processar a etapa de aceitação do termo
            // Por exemplo, você pode adicionar uma mensagem de sucesso ou redirecionar o usuário para outra página
            {
            ...
            // Se a etapa não for "accept", exiba o termo de contrato
            $twig->display('pagseguro.agreement.html.twig');
        }
    }
} elseif ($action == 'final') {
